...
class GaborQuery:
    ...
    def validate(self, min_cc, min_env, separability=None, inseperable_thresh=0.5, verbose=True):
        ...
        if verbose:
            logger.info(f"CC criteria exclusion {(self._query_cc(min_cc)==False).sum()}")
            logger.info(f"Location criteria exclusion {(self._query_location()==False).sum()}")
            logger.info(f"Envelope criteria exclusion {(self._query_envelope(min_env)==False).sum()}")
    ...
    def get_temporal_profile(self, min_cc=0.8, min_env=0.5, separability=None, inseperable_thresh=0.5, verbose=True, n_spatial_iterations=100, spatial_lr=1e-2, device="cuda"):
        # TODO: Return n x t temporal profiles and n relative responses
        params_df, spatial_rfs, gabors, strfs, rfs2d = self.validate(min_cc, min_env, separability=separability, inseperable_thresh=inseperable_thresh, verbose=verbose)

        n_dim, t_dim = strfs.shape[0], strfs.shape[1]
        gabor_params = GaborQuery._df_params_to_gabor_params(params_df)
        gabor_params = [torch.repeat_interleave(gabor_param, t_dim) for gabor_param in gabor_params]

        gabor_fitter = GaborFitter()
        gabor_model = gabor_fitter.fit_spatiotemporal(strfs.to(device), gabor_params, n_spatial_iterations, spatial_lr, device)
    ...
    def _query_separability(self, separability, inseperable_thresh):
        ...
        query = inseperable_units if separability == GaborQuery.SPACE_TIME_INSEPARABLE else inseperable_units == False
    ...
    def _get_correlation(self):
        ...
        return [correlation(gabor, rf) for gabor, rf in zip(self._gabors, self._spatial_rfs)]
