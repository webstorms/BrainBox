...
class VanRossum(nn.Module):
    ...
    def __init__(self, len, tau=20, dt=1):
        ...
        self.tau = tau
        self.dt = dt
        self.kernel = nn.Parameter(torch.Tensor([np.exp(-t/tau) for t in range(len-1, -1, -1)]).view(1, 1, -1), requires_grad=False)
...
class SpikeToPSTH(nn.Module):
    ...
    def __init__(self, len, sig=20):
        ...
        self.kernel = nn.Parameter(torch.Tensor([SpikeToPSTH.gaus(t, sig) for t in range(start_idx, end_idx)]).view(1, 1, -1), requires_grad=False)
